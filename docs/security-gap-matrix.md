...
| **D2** | **Goal:** structural DB-backed tenant isolation (RLS). **Runtime arch (Spike-B validated):** dedicated **non-`BYPASSRLS`** runtime role + separate migration/owner role + `@prisma/adapter-neon` (Neon serverless driver) + ALS tenant context + `SET LOCAL` in transaction + `FORCE` RLS, fail-closed | **Goal `LOCKED` · Runtime Arch `VALIDATED` (Spike B) · Impl: Phase 1 (runtime role provisioning) `COMPLETED / VERIFIED` in prod (2026-08-17; role `app_runtime` inert, runtime still owner) · RLS / adapter / role-switch / context-plumbing / Phase 2+ `NOT STARTED` · D2 overall `OPEN`** | `docs/security-d2-tenant-isolation-architecture-v1.md` (canonical; supersedes the earlier impact-review / decision-package refs, which are **not** in-repo) |
