...
- **`Equality`** — `RATIFIED · Primitive Operator #6 · member of Canonical Relation Evaluation · Equality/contract@v1-2026-08-11` (history: W1 COMPLETE 2026-07-22; EQ-CF1 class-fit 2026-07-29; W2 ACCEPTED §K 2026-08-06; W3 §L; Design Bundle §M; §2 Contract §N; W6 Closure Battery CLEAN PASS §O). **RATIFIED member** — Owner-ratified 2026-08-11 following W6 CLEAN PASS; operatorId `Equality`; §2 contract = §N; C1–C8 PASS.
...
### M · Equality v1 Design Bundle — DERIVED FROM §L (verbatim) · no §2 · Equality `CONTRACT-PENDING`

**§M.1 · Purpose.** This Design Bundle collects, **verbatim**, the owner-approved W3 design-axis locks for Equality v1 (§L, EA1–EE2) into one coherent artifact. It **adds, removes, interprets, resolves, refines, and rewords nothing**. Every clause in §M.2 is a verbatim copy of its source in §L; the Source Mapping (§M.6) proves one-to-one provenance. **This Bundle is Evidence, not Design**: it makes no new decision and creates no new normative wording. §L is not organized into separate Decision/Contract/Boundary groups; this Bundle preserves §L's single phased axis-lock structure and does not re-bucket it.

**§M.2 · Included Design-Axis Locks** *(verbatim from §L, EA1–EE2, Phase A–E)*
...
**§M.3 · Inherited Membership Projection.** *Registration of an inherited membership requirement projected from an external ratified authority into the Equality §2 Contract. §M holds registration metadata only — it does not hold the projected text and is not an authority.*

**IMP-2 · C4 Determinism & Statelessness**
- **Registration ID:** IMP-2
- **Authority / HOME:** C4 §2.5
- **Canonical Rendering:** CPT-1
- **Projection Target:** the Equality §2 determinism/statelessness requirement, to be materialized in the subsequent Equality §2 Contract record. (§2 is not yet written; no field number, name, or layout is asserted here.)
- **Projection Mode:** verbatim (CPT-1 → the Equality §2 determinism/statelessness requirement, applied when the §2 Contract is recorded)
- **Status:** inherited membership registration only. Reuses the existing operator-agnostic CPT-1; no new CPT; CPT-1 unchanged. Not a W3 lock, contract lock, authority, decision, or norm created in §M. The projection into the Equality §2 Contract is NOT performed here.

**§M.4 · Deferral Index** *(source-preserving · axis-by-axis · referential only; verbatim from §L; no deduplication, merge, normalization, rewording, or new Deferral)*
...
> **Assembly note — non-semantic (ED3):** §M.3 materializes the IMP registration portion of the ED3 deferred item. The verbatim projection into the Equality §2 Contract remains deferred to the subsequent §2 Contract record. This note does not alter, narrow, resolve, or reword the §L Deferred item.

**§M.5 · Acceptance Checklist** *(to be verified by the Bundle Audit)*
- [ ] No new decision.
- [ ] No new normative wording.
- [ ] No resolution, refinement, or reword.
- [ ] Every clause in §M.2 (EA1–EE2, with its Excludes and Deferred) is a verbatim copy of §L.
- [ ] No re-bucketing; Phase A–E order preserved.
- [ ] Every §L Deferral appears in §M.4 keyed to its source axis, unchanged, with no deduplication or merge.
- [ ] Every Source Mapping entry is one-to-one.
- [ ] IMP-2 registered exactly once with Authority/HOME, Rendering, Target, and Mode; no projected text held in §M; CPT-1 unchanged.
- [ ] No §2 Contract, no allocation, no ratification, no status/Ledger/CRE/Contracts-Register change.

**§M.6 · Source Mapping** *(one-to-one provenance)*

| §M element | Source | Category |
|---|---|---|
| §M.2 · EA1–EE2 (each lock, Excludes, Deferred) | §L · corresponding EAx (verbatim) | evidence (corpus) |
| §M.4 · Deferral Index (each entry) | §L · corresponding axis Deferred (verbatim) | evidence (corpus) |
| §M.3 · IMP-2 | CPT-1 (Authority / HOME: C4 §2.5) | inherited membership registration |
| §M structure | Ordinal §I — STRUCTURAL PRECEDENT ONLY (no semantic reuse) | template |
...
### N · Equality — §2 Contract · **RATIFIED — Primitive Operator #6 — Canonical Relation Evaluation** · `Equality/contract@v1-2026-08-11` (Owner-ratified 2026-08-11, following W6 Closure Battery CLEAN PASS §O)

> **Governance.** Structure = the Ratified §2 Template (11 fields). Content = the §M Bundle ONLY (verbatim). HOME = the single normative-authority site where a commitment is defined; a Reference is navigational only. Contract Metadata (§1–§2) is excluded from the Projection Audit. **Ratified 2026-08-11 following W6 Closure Battery CLEAN PASS (§O).** Ratification introduces no new decision, refinement, or Open-Question resolution; the semantic bodies (§3–§11) are unchanged from the W6-closed draft, and only the Contract Metadata (§1–§2 / header) transitions to the allocated identifiers.

**Contract Metadata (not Projection; excluded from Projection Audit)**

1. `operatorId` — **`Equality`** (assigned at Ratification, 2026-08-11). Stable normative identity; PascalCase per the existing scheme; contains no version (version lives in the Contract ID).
2. `class` — **RATIFIED METADATA.** `Canonical Relation Evaluation` (PART A §6b, ratified `@v1.1`).
...
**Governance Note.** Ratified 2026-08-11 as **Primitive Operator #6 — Canonical Relation Evaluation**, contract `Equality/contract@v1-2026-08-11`, following the W6 Closure Battery CLEAN PASS recorded in §O. Ratification projects §M verbatim and writes no new normative wording; it allocates the operatorId (`Equality`), Primitive number (#6), and Contract ID, and adds the corresponding §4 Ledger row, CRE Register membership, and Ratified Contracts Register entry. The W6 evidence (§O) remains immutable; the 2 OUT-OF-SCOPE items (Coverage OQ-39, Composition) and all §11 Deferrals remain open and non-blocking.
...
| **Equality** | L1 | **RATIFIED — Primitive Operator #6 — Canonical Relation Evaluation** | `Equality/contract@v1-2026-08-11` (§N). **Owner-authorized ratification 2026-08-11 following W6 Closure Battery CLEAN PASS §O** (C1–C8 PASS; Cross-layer PASS; 2 OUT-OF-SCOPE justified). **Second ratified member of Canonical Relation Evaluation** (after Ordinal #5). operatorId `Equality`. Canonical value-equality of two peer relata over ONE authorized, version-pinned equivalence domain → `EQUAL`/`NOT_EQUAL` (EA1/EC1); Outcome XOR Failure; inadmissibility→Failure, never `NOT_EQUAL` (EB3/EC2); determinism/statelessness (pure function; no state/schedule/inference/side effects) projected from C4 §2.5 via CPT-1 (IMP-2) into §N field 8. C2 via single CRE assignment (Amendment A3); C4 explicit via Amendment A4 + CPT-1 + IMP-2. Out-of-contract: Coverage (OQ-39), Composition (OPEN); §11 Deferrals remain open/non-blocking. Historical Equality candidate row preserved; Primitives #1–#5 unchanged; no new class; no Primitive renumbering. | 2026-08-11 |
...
- **Ratified members:** `Ordinal` — Primitive Operator #5, `Ordinal/contract@v1-2026-07-31`, ratified 2026-07-31 (first member of this class) · `Equality` — Primitive Operator #6, `Equality/contract@v1-2026-08-11`, ratified 2026-08-11 (second member of this class)
- **Active candidates:** none. (`Ordinal` — Primitive #5 — and `Equality` — Primitive #6 — are now ratified members, above; neither remains an active candidate.)
- **Composition:** `OPEN`
...
- **`Equality/contract@v1-2026-08-11`** — Primitive Operator #6, class **Canonical Relation Evaluation** (the SECOND member of the third class). Frozen text = §N (11 fields; §2 Contract projected verbatim from Bundle §M). **Purpose:** deterministic evaluation of canonical value-equality between two peer relata over one authorized equivalence domain, emitting exactly one of two Canonical Relation States `EQUAL` / `NOT_EQUAL` (EA1/EC1). **Inputs:** two peer relata of the same authorized, version-pinned equivalence domain; admissibility decided upstream; canonicalization Domain-owned and optional; no membership inference (EA2/EA3/EB1/EB2/EB3/EB4). **Output:** two Canonical Relation States; Outcome XOR Failure; inadmissibility → Failure, never `NOT_EQUAL`; no fabricated outcome (EC1/EC2). **Identity:** subject = the equality operation; operation-identity and outcome/result-record identity are distinct (ED1/ED2). **Determinism/statelessness:** pure function; no state / schedule / inference / side effects — projected verbatim from C4 §2.5 via CPT-1 (registered by IMP-2) into §N field 8; plus ED3. **Failure:** families A/B/C/D — A is Equality-owned; B/C/D are recognized boundaries governed by the violated invariant / supervisory authority (EE1/EE2). Out of contract: Coverage (OQ-39), Composition (OPEN), and the §11 implementation Deferrals. W6 Closure Battery CLEAN PASS (§O); DEFECT-free; C2 via single CRE assignment (Amendment A3); C4 explicit via Amendment A4 + CPT-1 + IMP-2. Semantic change routes through versioningRule (§N field 10).
