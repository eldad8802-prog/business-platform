...
> This document materializes the already-ratified RIA-1 §0–§10 semantic contract as the in-repo source of authority. It introduces **no** new semantic decisions, changes **no** locks, and defines **no** production matching heuristics, DB representation, APIs, or feature wiring. Per-turn verification scans, ratification checklists, and drafting recommendations from the ratification dialogue are intentionally omitted (canonical corpus, not transcript); every normative lock, OPEN/DEFERRED item, and guard is preserved with its ratified identifier.
...
# RIA-1 · §10 — Method-Policy Representation, Registry, Versioning & Selection
*Prefix: `RP`. §0–§9 = INHERITED. Representation-semantic contract, **not** storage schema: how a §8/§9 Method Policy is identified, versioned, represented, selected, and replayed without the store or runtime becoming an authority source.*

## §10 Normative Locks (RP1–RP19)

**A · Policy Lineage & Version**
- **RP1 · Policy Lineage Identity** — A Method Policy has a stable **Policy Lineage Identity** — the continuous normative identity of the policy across evolution — identifying **the one semantic authorization question the lineage exists to govern**. It is distinct from any version. *(PA2/MP3 · newly-ratified)*
- **RP2 · Policy Version** — A **Policy Version** is an **immutable** normative snapshot of one lineage at a point, identified by *(Lineage Identity, version)*. Historical Assertions stay pinned to the version in force at their time. *(MP3/RA18/TR20 · inherited)*
- **RP3 · Version-evolution scope** — A change to conditions/guards/verification-source/freshness/lifecycle-conditions/effective-applicability/admitted-grounds/authorization-conditions **MAY** be a new **version** of the same lineage, **provided it does not change the semantic authorization question the lineage governs**. *(SD1 · newly-ratified)*
- **RP4 · New-Lineage Semantic Boundary** — A new Policy Lineage is REQUIRED when a change alters the **semantic authorization responsibility** of the policy rather than merely evolving the conditions under which the same authorization responsibility applies. At minimum, a change of **(1) Referent Type · (2) Authorization Question / decision-category · (3) identifier namespace / governed identity class · (4) fundamental policy purpose** constitutes such a semantic-boundary crossing and MUST NOT masquerade as ordinary version evolution. Changes to **tenant, jurisdiction, method/source qualification, temporal applicability, verification requirements, guards, freshness, lifecycle conditions, admitted grounds, or authorization conditions do not by themselves require a new lineage** and MAY be represented as version/applicability evolution where the governed semantic authorization responsibility remains unchanged. They MUST NOT, however, be declared **categorically incapable** of participating in a future semantic-boundary determination if changing them **also** changes the underlying authorization responsibility. The determination MUST be **governance-defined and deterministic**; it MUST NOT be left to implementation convenience, DB-key construction, or runtime judgment. *(SD1 corrected · newly-ratified; compound-change OPEN)*
- **RP5 · Identity anti-mechanization + meaning-preservation** — Policy Lineage Identity **MUST NOT** be derived mechanically from a serialized scope-signature / hash / schema composite key — it is a **governed semantic lineage, not a DB key**. And **versioning MUST NOT change the governed Authorization Question while making historical and current versions appear semantically interchangeable.** *(SD1 · newly-ratified)*

**B · Representation Content**
- **RP6 · Representation content** — A Policy Version's representation MUST express: lineage identity + version · scope (tenant, Referent Type/domain-qualification, decision-categories, method(s), purpose, jurisdiction where relevant) · temporal applicability · the **§8 PA16 identifier-authority condition slots** it conditions on · the **§9 authorization conditions** (per identifier × decision-category) · provenance. Serialization/schema **DEFERRED**. *(PA2/PA16/§9/PA5 · inherited)*
- **RP7 · Version = normative unit; rule provenance, no rule-hierarchy** — The **Policy Version** is the pinned normative unit (RA18); Basis provenance (PA23) **MUST preserve the contributing conditions/grounds**; **no independent versioned rule artifact is introduced.** A stable rule-key MAY remain future-compatible but is not required. *(RA18/PA23/SD2 · clarification-derived)*

**C · Registry & Governance Ownership**
- **RP8 · Registry semantic responsibilities** — A **Method-Policy Registry** (a semantic responsibility, not a table) MUST enable: enumerate candidate applicable policies for a pinned context · resolve exact *(lineage, version)* · deterministic lookup · historical-version access · **prevent feature-owned shadow policies**. Implementation **DEFERRED**. *(MP19/PA17/MP23 · inherited)*
- **RP9 · Governance ownership** — **Register · publish-version · retire/supersede · change-scope** are **governance-authorized acts**. **Feature code MUST NOT register a policy or publish a version.** *(MP23/RA17 · clarification-derived)*

**D · Selection & Multi-Applicability**
- **RP10 · Deterministic Evaluation-Plan selection** — Selection **MUST deterministically resolve to exactly one governed Evaluation Plan** for one pinned Authorization Question + context, **determined before observing the outcome**, **not feature-chosen**, **no outcome-shopping**. *(PA17/MP19/SD3 · inherited)*
- **RP11 · Evaluation Plan meaning** — An **Evaluation Plan** is the deterministic *result* of selection. In the **current** contract it resolves to a **single Method Policy Version**. §10 **neither claims inter-policy composition exists nor forbids** a future explicitly-ratified composition contract; composition may become a governed form only through explicit ratification. *(SD3/MP2/MP7 · newly-ratified framing; composition OPEN)*
- **RP12 · Unresolved multi-applicability surfacing** — If more than one policy is applicable to one Authorization Question with no governed resolution, selection **MUST surface an explicit, deterministic, pre-evaluation disposition**. It **MUST NOT** become first-match / latest-version / most-specific / insertion-order / implicit-precedence / arbitrary selection. **The exact selection-layer disposition category remains OPEN** (deferred to the failure/disposition taxonomy) — and this OPEN is **visible here, not hidden**. *(PA14/PA17/RA22/PA26/SD4 · partly forced + explicit OPEN)*

**E · Applicability & Temporal**
- **RP13 · Applicability derivation** — A policy's applicability to a context is **derived** from its scope + temporal semantics against the pinned context/EvaluationTime — **not** from an `enabled` flag, latest-version, highest number, or insertion order. *(PA17/PA20/TR10 · clarification-derived)*
- **RP14 · Temporal distinctness + no auto-reprocess** — Recorded-time, effective-time, and historical execution are **distinct** (§6). A **later-recorded / earlier-effective** Policy Version **MUST NOT automatically reinterpret or reprocess** historical Assertions or historical execution (TR6/TR20); historical replay **pins the version-in-force**. **Backdating capability and explicit reprocessing remain separate governed operations (OPEN).** *(§6/TR2/TR6/TR20/MP20/SD5 · inherited)*

**F · Replay / Reconstructibility**
- **RP15 · Historical-Selection Reconstructibility** — Historical policy selection MUST be reconstructible/replayable **to the degree required to reproduce and audit the governed selection decision under the pinned historical selection context**, including the selected `(lineage, version)`, the applicable temporal/context inputs, and the governed selection basis. §10 **does not prescribe** that this requires reconstruction of the complete registry universe, a stored candidate set, a registry snapshot, digest, manifest, or any particular encoding, **unless a later contract establishes** that such information is necessary for deterministic reproduction. *(PA21/TR25/MP19/SD6 corrected · partly forced + artifact OPEN)*

**G · Retirement / Supersession**
- **RP16 · Retirement / Supersession** — Retirement/supersession is **append-only, not deletion**; historical versions remain **resolvable and replayable**; retirement **does not rewrite historical Assertions**. Retirement/supersession changes policy applicability/selection only according to its **governed temporal semantics and the pinned reconstruction question**; it **does not rewrite Historical-Execution-Replay or previously recorded Assertions**. A successor **must independently satisfy the applicable scope, temporal-applicability, and selection contract**; **successor designation is never precedence** (`P2 supersedes P1` ≠ `P2 wins`). Whether retirement may itself carry **future-effective or backdated-effective** semantics remains **OPEN** except where already constrained by §6. *(MP20/§3/TR/RA22/PA17/SD7 corrected · inherited + OPEN)*

**H · Layering & Runtime Boundary**
- **RP17 · Layer non-collapse** — The chain **Policy Lineage → Policy Version → applicability → Evaluation-Plan selection → Authorization Evaluation → Basis / No-Authorization / governed Failure → Assertion → Identity State** MUST NOT collapse adjacent layers: **Policy identity ≠ version ≠ applicability ≠ selection ≠ evaluation ≠ Basis ≠ Assertion.** *(§8 cross-SD/PA26 · newly-ratified)*
- **RP18 · Runtime hand-off boundary** — §10 supplies the contract for Runtime to *load → deterministically select the Evaluation Plan → pin → evaluate → produce Basis/No-Auth/Failure* **without the store or runtime deciding** identifier authority, selection precedence, version applicability, or scope. Runtime/persistence/evaluator **DEFERRED**. *(PA17/PA22/PA25 · clarification-derived)*
- **RP19 · Forbidden representation shortcuts** — Blocked: `DB-row = policy identity` · `latest/highest-version wins` · `enabled-flag = applicability` · `most-specific wins (unratified)` · `first-registry-match` · `insertion-order = selection` · `feature registers private policy` · `mutable policy version` · `policy edit rewrites historical assertion meaning` · `current registry state used for historical replay` · `schema default = authority semantics`. *(RA22/PA17/PA25/MP20 · inherited consolidation)*

## §10 OPEN / DEFERRED
selection-ambiguity **disposition category** (RP12) · **compound-change semantic-boundary determination** (RP4) · historical-selection **reproduction evidence mechanism/artifact** (RP15 — snapshot/candidate-set/digest DEFERRED unless later proven necessary for deterministic reproduction) · **retirement future/backdated-effective** semantics (RP16) · inter-policy **composition** (RP11) · registry/DB/serialization **implementation** · runtime evaluator/persistence · **PA14(b)** · concrete **precedence** (RA22) · Party migration mechanism · concrete Minting · feature wiring · CONFLICT adjudication · verification APIs · **כל §0–§9 OPEN/DEFERRED**.
...
- Method-Policy **representation contract ratified (§10 RP1–RP19)**; still OPEN: registry/serialization/**storage implementation** (RP6/RP8), selection-ambiguity **disposition category** (RP12), **compound-change lineage boundary** (RP4), historical-selection **reproduction artifact** (RP15), **retirement future/backdated-effective** (RP16), **inter-policy composition** (RP11).
...
**§10 materialization (non-semantic transcription):** §10 (Method-Policy Representation / Registry / Versioning / Selection, RP1–RP19) is transcribed verbatim from its ratified in-conversation draft, including the ratified **corrected RP4** (New-Lineage Semantic Boundary — four minimal splitters; other qualifiers not automatic splitters; compound-change OPEN; governance-defined, not runtime-judgment), **corrected RP15** (Historical-Selection Reconstructibility as a reproduce/audit requirement; snapshot/candidate-set/digest artifact DEFERRED), and **corrected RP16** (append-only retirement; applicability per governed temporal semantics; successor ≠ precedence; future/backdated-effective OPEN). No new decisions; identifiers RP1–RP19 preserved; §10 OPEN/DEFERRED items retained as OPEN.
