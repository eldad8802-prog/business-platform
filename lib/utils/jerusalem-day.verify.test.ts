...
function main() {
  ...
  // 21:30 UTC in winter is 23:30 Israel (UTC+2) — still the same day.
  eq(
  ...
  // Forward transition (late March) and back (late October). A calendar span is
  // unaffected by both, because it never measures elapsed time.
  eq(
