/**
 * Billing / Invoices — Dubiz Design System v1 theme (warm cream / teal).
 *
 * PRESENTATION ONLY. The Billing feature was authored on the cool navy/slate
 * `TOKEN` set; the live product reference (Payment Secretary + Collection +
 * Documents + Inventory) is the warm DS v1 language (`TOKEN.dsv1`). This module
 * re-skins Billing onto DS v1 from ONE place: it re-exports a `TOKEN`-shaped
 * object whose surfaces, ink, borders, brand, semantic signals, radii, weights
 * and shadows are mapped to the DS v1 source-of-truth, plus warm variants of the
 * shared action styles.
 *
 * Every Billing file swaps its imports from `@/lib/design/tokens` /
 * `@/lib/design/action-styles` to this module — no call sites change, so the
 * palette flips centrally exactly like the Documents theme.
 *
 ...
 *
 * NOT a source of truth for the invoice PDF: the legal document is generated
 * server-side. Representational previews (e.g. BillingDocumentStylePicker's
 * MiniPreview) must keep their own colours and are NOT re-skinned via this theme.
 */
...
export const TOKEN = {
  ...
  surface: {
    // Near-black chrome stays as-is (rail/app-level, outside the Billing skin).
    appChrome: BASE.surface.appChrome,
    /** Page canvas — warm cream. */
    page: d.canvas,
    /** Cards, panels, banners — warm paper. */
    card: d.card,
    /** Inset surface inside cards (inputs, tiles, line-editor containers). */
    inset: d.surface2,
...
export function chipActionStyle(active = false): CSSProperties {
  return {
    ...
    background: active ? d.accentSoft : TOKEN.action.glass.background,
