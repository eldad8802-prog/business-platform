...
async function main() {
  ...
  // The REAL guard, unmocked. Every case above injects `authorize`, which proves
  // the handler honours a refusal but not that the refusal happens. These two
  // leave the default in place, so what runs is the same
  // requirePlatformAdminOrResponse every other admin route calls — and the probe
  // must not reach the database on either.
  {
    let reads = 0;
    const spy = async () => {
      reads += 1;
    ...
    const noHeader = await handleAuthPlaneIdentityProbe(req(), { readCurrentUser: spy });
    ok("REAL guard: no Authorization header -> 401", noHeader.status === 401);

    const bogus = await handleAuthPlaneIdentityProbe(
      new NextRequest(
        "https://app.test/api/platform-admin/diagnostics/auth-plane-identity",
        { method: "POST", headers: { authorization: "Bearer not.a.valid.token" } }
      ),
      { readCurrentUser: spy }
    );
    ok("REAL guard: an unusable bearer token -> 401", bogus.status === 401);

    ok("REAL guard: the database was never reached", reads === 0);
  }
