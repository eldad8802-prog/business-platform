...
export default function EmailDocumentsPage() {
  ...
  async function refreshStatus() {
    ...
    try {
      ...
      setConnected(null);
      setStatusError(e?.message || "שגיאה בסטטוס Gmail");
    } finally {
  ...
  async function startConnect() {
    ...
    try {
      ...
      setStatusError(e?.message || "שגיאה בחיבור Gmail");
    }
  }
  ...
  return (
    <div dir="rtl" style={{ minHeight: "100vh", background: "#f8fafc" }}>
      <DocumentsHeader title="מסמכים מהמייל" />

      <main style={pageMain}>
        <section style={hero}>
          <div style={heroKicker}>ייבוא ממייל</div>
          <h1 style={heroTitle}>מסמכים שנמצאו במייל</h1>
          <p style={heroSubText}>
            סורקים את התיבה שלך, מציגים מה נמצא, ואתה בוחר מה לייבא.
          </p>
        </section>
