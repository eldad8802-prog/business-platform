...
--   app_admin      : SELECT only. The platform-admin features screen reads
--                    across tenants through p7adm_read. The read-only admin
--                    doctrine is UNCHANGED — no INSERT/UPDATE/DELETE is added
--                    to app_admin anywhere in this wave.
...
GRANT USAGE, SELECT ON SEQUENCE "PlatformAuditEvent_id_seq" TO app_ctlplane;
