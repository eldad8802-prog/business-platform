...
async function measure(page) {
  return page.evaluate(() => {
    ...
    // The column is marked now; fall back to the first real child for the hub,
    // which has no marker. A <style> node measures 0 and is not the column.
    const col = document.querySelector("[data-billing-column]") ||
      (main ? Array.from(main.children).find((e) => e.tagName !== "STYLE") : null);
    const rect = (el) => {
      if (!el) return null;
    ...
    const stageRegion = document.querySelector("[data-wsl] > .wsl-end");
    ...
    return {
      ...
      stage,
      pinned,
      rail: rail && getComputedStyle(rail).display !== "none" ? rect(rail) : null,
      stageRegion: stageRegion && getComputedStyle(stageRegion).display !== "none" ? rect(stageRegion) : null,
  ...
  for (const [name, route] of ROUTES) {
    for (const w of VIEWPORTS) {
      ...
      console.log(
        ...
        " rail=" + (m.rail ? m.rail.w + "@x" + m.rail.x : "-") + " stage=" + (m.stageRegion ? m.stageRegion.w + "@x" + m.stageRegion.x : "-") + " pinned=" + m.pinned.length + " minTap=" + m.minTap +
